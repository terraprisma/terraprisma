...
internal class WinImm32Ime : PlatformIme, IMessageFilter
{
	...
	public uint SelectedPage => _candSelection == 0 ? 0 : _candSelection / _candPageSize;
	...
	private void UpdateCandidateList()
	{
		...
		try {
			...
			ref CandidateList candList = ref AsRef<CandidateList>(buf);
	...
	private static ref T AsRef<T>(ReadOnlySpan<byte> span) where T : struct
	{
